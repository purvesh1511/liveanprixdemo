...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="overflow-x-auto">
        <table id="contact-inquiries-table" class="w-full text-sm datatable" data-order='[[4, "desc"]]'>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="{{ !$inquiry->is_read ? 'bg-blue-50/30' : '' }}">
                    ...
                    <td class="px-6 py-4 text-right whitespace-nowrap">
                        <a href="{{ route('admin.contact-inquiries.show', $inquiry) }}" class="text-blue-500 hover:text-blue-700 text-sm mr-3"><i class="fas fa-eye"></i> View</a>
                        @if(!$inquiry->is_read)
                        <form action="{{ route('admin.contact-inquiries.read', $inquiry) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                ...
                @empty
                <tr><td colspan="6" class="px-6 py-8 text-center text-gray-400">No contact inquiries found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
